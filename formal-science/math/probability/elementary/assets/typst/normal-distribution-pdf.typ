...
#align(center)[
  #figure(
    cetz.canvas(length: 20pt, {
      ...
      let normal-distribution-pdf(t, deviation, mean) = (
        1 / (deviation * calc.sqrt(2 * calc.pi)) * calc.exp(
          - calc.pow(t - mean, 2) / (2 * calc.pow(deviation, 2))
        )
      )
          ...
          for (idx, i) in deviations.enumerate() {
            for j in means {
              if j != 0 {
                plot.add(
                  domain: (-5, 5),
                  t => normal-distribution-pdf(t, i, j),
                  style: (
                    stroke: (
